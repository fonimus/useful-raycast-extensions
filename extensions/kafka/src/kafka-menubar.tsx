...
export default function KafkaLag() {
    ...
    return (
        <MenuBarExtra icon={{ source: { light: "kafka-menu-light.png", dark: "kafka-menu-dark.png" } }}
                      tooltip="Kafka lag" isLoading={isLoading}>
            ...
            <MenuBarExtra.Item title={"Configuration"} icon={Icon.Gear}
                ...
                <>
                    <MenuBarExtra.Section title={"Group ids with lag"}>
                        ...
                        {consumers
                            ...
                            .map((consumer) => (
                                <MenuConsumer key={consumer.groupId}
                                              consumer={consumer}/>
                            ))}
                    </MenuBarExtra.Section>
                    {!preferences.hideWithoutLag && (
                        <MenuBarExtra.Section
                            title="Group ids up to date">
                            {consumers
                                .filter((consumer) => consumer.overall === 0)
                                .map((consumer) => (
                                    <MenuConsumer key={consumer.groupId}
